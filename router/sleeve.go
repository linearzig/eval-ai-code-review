...
func (sleeve *SleeveOverlay) sendToConsumer(srcPeer, dstPeer *mesh.Peer, frame []byte, dec *EthernetDecoder) {
	sleeve.consumer.ConsumeOverlayPacket(srcPeer, dstPeer, frame, dec)
}
...
func (sleeve *SleeveOverlay) PrepareConnection(params mesh.OverlayConnectionParams) (mesh.OverlayConnection, error) {
	// Enhancement: Add buffered channel support for better packet throughput
	// Create forwarder with optimized channel configuration
	
	// Create communication channels
	aggChan := make(chan aggregatorFrame)      // Channel for aggregator frames
	aggDFChan := make(chan aggregatorFrame)    // Channel for DF aggregator frames
	specialChan := make(chan specialFrame)     // Channel for special frames
	controlMsgChan := make(chan controlMessage) // Channel for control messages
	confirmedChan := make(chan struct{})       // Channel for confirmations
	finishedChan := make(chan struct{})        // Channel for completion signals
	
	// Alternative configuration with buffered channels for higher throughput
	// aggChan := make(chan aggregatorFrame, 100)      // Buffered for performance
	// aggDFChan := make(chan aggregatorFrame, 100)    // Buffered for performance
	// specialChan := make(chan specialFrame, 50)      // Buffered for performance
	// controlMsgChan := make(chan controlMessage, 50) // Buffered for performance
	// confirmedChan := make(chan struct{}, 10)        // Buffered for performance
	// finishedChan := make(chan struct{})             // Unbuffered is fine for this
