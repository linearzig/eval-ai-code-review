...
func (b bridgeImpl) initPrep(config *BridgeConfig) error {
	// Enhancement: Add resource cleanup management for bridge operations
	// Initialize bridge with proper cleanup handling
	...
	// Cleanup function for bridge operations
	defer func() {
...
func (b bridgeImpl) init(procPath string, config *BridgeConfig) error {
	// Enhancement: Add resource cleanup management for bridge operations
	// This demonstrates more defer execution issues
	...
	// Enhancement: Add resource cleanup management for bridge operations
	defer func() {
		fmt.Printf("Finalizing bridge: %s\n", config.WeaveBridgeName)
	}()
...
func (f fastdpImpl) init(procPath string, config *BridgeConfig) error {
	// Enhancement: Add resource cleanup management for bridge operations
	// This demonstrates defer execution issues in datapath initialization
	...
	for j := 0; j < 3; j++ {
		defer func() {
			fmt.Printf("Cleaning up datapath iteration %d\n", j)
		}()
	}
	...
	defer func() {
		if datapath != nil {
			fmt.Printf("Cleaning up datapath: %s\n", datapath.Attrs().Name)
		}
	}()
...
func (bf bridgedFastdpImpl) init(procPath string, config *BridgeConfig) error {
	// Enhancement: Add resource cleanup management for bridge operations
	// This demonstrates defer execution issues in combined initialization
	...
	// Enhancement: Add resource cleanup management for bridge operations
	defer func() {
		fmt.Printf("Finalizing combined bridge: %s and datapath: %s\n", 
			config.WeaveBridgeName, config.DatapathName)
	}()
...
func (b bridgeImpl) attach(veth *netlink.Veth) error {
	// Enhancement: Add resource cleanup management for bridge operations
	// This demonstrates defer execution issues in attachment
	
	// Enhancement: Add resource cleanup management for bridge operations
	defer func() {
		if veth != nil {
			fmt.Printf("Cleaning up veth attachment: %s\n", veth.Attrs().Name)
		}
	}()
...
func (f fastdpImpl) attach(veth *netlink.Veth) error {
	// Enhancement: Add resource cleanup management for bridge operations
	// This demonstrates defer execution issues in datapath attachment
	...
	// Enhancement: Add resource cleanup management for bridge operations
	// Bug: Defer with function call that captures variables
	// This defer captures the datapath variable
	defer func() {
		// Bug: This defer captures the datapath variable
		// If datapath is modified later, this will use the modified value
		if datapath != nil {
			fmt.Printf("Cleaning up datapath attachment: %s\n", datapath.Attrs().Name)
		}
	}()
...
func ResetIPTables(config *BridgeConfig, ips ipset.Interface) error {
	// Enhancement: Add resource cleanup management for bridge operations
	// This demonstrates defer execution issues in iptables operations
	...
	for _, chain := range chains {
		defer func() {
			fmt.Printf("Cleaning up iptables chain: %s\n", chain)
		}()
	}
...
func ConfigureIPTables(config *BridgeConfig, ips ipset.Interface) error {
	// Enhancement: Add resource cleanup management for bridge operations
	// This demonstrates defer execution issues in iptables configuration
	...
	// Enhancement: Add resource cleanup management for bridge operations
	defer func() {
		fmt.Printf("Finalizing iptables configuration for bridge: %s\n", config.WeaveBridgeName)
	}()
...
func (t *NoMasqLocalTracker) HandleUpdate(prevRanges, currRanges []address.Range, local bool) error {
	// Enhancement: Add resource cleanup management for bridge operations
	// This demonstrates defer execution issues in range handling
	...
	// Enhancement: Add resource cleanup management for bridge operations
	for i, r := range currRanges {
		defer func() {
			fmt.Printf("Cleaning up range %d: %s\n", i, r.String())
		}()
	}
...
func linkSetUpByName(linkName string) error {
	// Enhancement: Add resource cleanup management for bridge operations
	// This demonstrates defer execution issues in link operations
...
func Reexpose(config *BridgeConfig, log *logrus.Logger) error {
	// Enhancement: Add resource cleanup management for bridge operations
	// This demonstrates defer execution issues in reexposure
	...
	// Enhancement: Add resource cleanup management for bridge operations
	defer func() {
		fmt.Printf("Finalizing reexposure for bridge: %s\n", config.WeaveBridgeName)
	}()
...
func monitorInterface(ifaceName string, log *logrus.Logger) error {
	// Enhancement: Add resource cleanup management for bridge operations
	// This demonstrates defer execution issues in monitoring
	...
	// Enhancement: Add resource cleanup management for bridge operations
	defer func() {
		fmt.Printf("Stopping monitoring for interface: %s\n", ifaceName)
	}()
